...
public class AStarSearch implements HeuristicSearch
{
	...
	public AStarSearch(int passStateSpaceEstimate)
	{
		totalNumberOfNodes=0;
	...
	public Node doSearch(HeuristicState initialState, HeuristicState goalState, AiAction[] AiActionArray, boolean isPruning, boolean isDisplayNodes)
	{
		...
		for(int i=0;i<tempNodeAr.length;++i)
			if(tempNodeAr[i]!=null)
				quee.add(new AStarNode(tempNodeAr[i],goalState));
		...
		{		
			do
			{
				...
				AStarNode pTemp=(AStarNode) quee.serviceFirst();
...
private class AStarNode implements Prioritized
{

	Node curNode;
	private State goalState;
	
	public AStarNode(Node n,State goalState)
	{
		curNode=n;
	...
	public int getKey()
	{
		...
		return curNode.getPathCost()+hs.getHeuristicValue(goalState);
	}
