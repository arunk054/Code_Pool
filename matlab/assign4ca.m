...
%% Q. 4 (Matlab)
%% Quantize the data (from parts (a)) using mu law quantizer and plot the SQNR
...
display('Executing assignment - 1 question 4(c) for uniform distribution...');
...
%% Generate the random variable uniformaly distributed u(-1,1)
...
%% define the mu law compander and expander formula
display(' ');
...
for bits=1:size(N)
    ...
    %Compute the noise by the diff between actual signal val and quanti val
     for i=1:sample_size
         % First apply the expander on the quantized value ie decoding
...
xlabel('No. of Bits ==>');
ylabel('SQNR in DB ==>');
title('Plot of SQNR vs no. of bits for a uniformly distributed signal using mu law quantization');
